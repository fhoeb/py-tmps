"""
    Test for star propagation of a pure quantum state represented by a mps
"""
import pytest
...
def test_regular_chain(L, system_index, sot, nof_steps, tau):
    ...
    tm_prop = from_hamiltonian(tm_state, 'mps', system_index, tm_site_ops, tm_bond_ops, tau=tau,
                               op_compression_kwargs=op_compression, second_order_trotter=sot,
                               state_compression_kwargs=state_compression)

    ed_prop = StarExDiagPropagator(exdiag_state, dims, system_index, ed_site_ops, ed_bond_ops, tau)
    normdist = []
    for i in range(nof_steps):
        tm_prop.fast_evolve(True)
        ed_prop.evolve()
        tmps_psi_t_array = mp.MPArray.to_array(tm_prop.psi_t).reshape(site_dim ** L)
...
def test_irregular_chain(L, system_index, sot, nof_steps, tau):
    """
        ...
        Tests are performed for different chain lengths and different time discretizations
        The initial state is random state
    :param L: Chain L
    :param system_index: Index of the system site in the chain
    :param sot: second order trotter evolution switch
    :param nof_steps: Number of time evolution steps
    ...
    for i in range(nof_steps):
        tm_prop.evolve()
        ed_prop.evolve()
        tmps_psi_t_array = mp.MPArray.to_array(tm_prop.psi_t).reshape(np.prod(dims, dtype=int))
        diff = tmps_psi_t_array - ed_prop.psi_t
        normdist.append(np.linalg.norm(diff))
    max_normdist = np.max(np.array(normdist))
    if sot:
