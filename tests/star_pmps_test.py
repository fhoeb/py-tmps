"""
    Test for star propagation of a mixed quantum state represented by a pmps
"""
import pytest
...
def test_regular_chain(L, system_index, sot, nof_steps, tau):
    ...
    exdiag_state, dims, ed_site_ops, ed_bond_ops, tm_state, tm_site_ops, tm_bond_ops = \
        star_test_generator.generate_regular_pmps_test(L, system_index, sys_site_op, bath_site_op,
                                                       sys_couple_op, bath_couple_op, seed=seed, rank=rank)
    op_compression = {'method': 'svd', 'relerr': 1e-12}
    state_compression = {'method': 'svd', 'relerr': 1e-10, 'sites_relerr': 1e-12}

    tm_prop = from_hamiltonian(tm_state, 'pmps', system_index, tm_site_ops, tm_bond_ops, tau=tau,
    ...
    for i in range(nof_steps):
        tm_prop.evolve()
        ed_prop.evolve()
        tmps_psi_t_array = mp.MPArray.to_array_global(mp.pmps_to_mpo(tm_prop.psi_t)).reshape(site_dim ** L,
                                                                                             site_dim ** L)
...
def test_irregular_chain(L, system_index, sot, nof_steps, tau):
    ...
    for i in range(nof_steps):
        tm_prop.evolve()
        ed_prop.evolve()
        tmps_psi_t_array = mp.MPArray.to_array_global(mp.pmps_to_mpo(tm_prop.psi_t)).reshape(np.prod(dims, dtype=int),
