...
class StarMPS4OPropagator(StarMPPropagatorBase):
    ...
    def _mpo_from_hi(self, hi, tau, lbond, rbond):
        """
            Builds e^(-1j*\tau*hi) for the physical legs and generates the mpo.
    ...
    def _get_full_sweep(self, trotter_exponentials, post_swap, pre_swap):
        ...
        full_sweep.append((self.system_index, trotter_exponentials[self.system_index][0]))
        ...
        full_sweep.append((self.system_index, trotter_exponentials[self.system_index][1]))
        return full_sweep
