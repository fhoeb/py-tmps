"""
    Imaginary time evolution class. Essentially the same as StarTPMPS class except for the lack of overlap tracking.
"""
import numpy as np
import mpnum as mp
...
class StarITPMPS(StarTPMPS):
    ...
    def from_hamiltonian(cls, psi_0, ancilla_sites, build_adj, system_index, h_site, h_bond, tau=0.01,
        ...
        """
        tmp = super(StarTPMPS, cls).from_hamiltonian(psi_0, ancilla_sites, build_adj, system_index, h_site, h_bond,
                                                     tau=tau,
    ...
    def from_hi(cls, psi_0, ancilla_sites, build_adj, system_index, hi_list, tau=0.01, state_compression_kwargs=None,
        ...
        """
            Same general form as the parent class constructor, with an additional option to track the trace
        """
        tmp = super(StarTPMPS, cls).from_hi(psi_0, ancilla_sites, build_adj, system_index, hi_list, tau=tau,
                                            state_compression_kwargs=state_compression_kwargs,
                                            op_compression_kwargs=op_compression_kwargs,
                                            second_order_trotter=second_order_trotter,
                                            t0=t0, force_op_cform=force_op_cform,
